...
#get case dates
end_date_raw<-max(case_nat_tbl$cases_date)
end_Bdate<-format(end_date_raw,format="%B %d, %Y")
end_date<-format(end_date_raw,format="%m/%d/%y")

#set case title
casetitle<-data.frame(title=if_else(params$timeframe=="Monthly","Monthly Change in COVID-19 Cases, US", "Weekly Change in COVID-19 Cases, US"),subtitle=glue("January 22, 2020 - {end_Bdate}"))%>%
...
#set death title
deathtitle<-data.frame(title=if_else(params$timeframe=="Monthly","Monthly Change in COVID-19 Deathss, US", "Weekly Change in COVID-19 Deaths, US"),subtitle=glue("January 22, 2020 - {end_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
...
table_bind <- bind_cols(cases_numbers,
                      deaths_numbers,
                    hosp_nat_tbl|> rename(hosp_daily=hosp_new)|>mutate(hosp_new=rollsum(hosp_daily,7,align="right",fill=NA))|>filter(hosp_date ==max(hosp_date)), 
