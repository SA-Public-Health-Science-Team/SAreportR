...
#set case title
casetitle<-data.frame(title=if_else(params$timeframe=="Monthly",glue("Monthly Change in COVID-19 Cases, {params$juris}"), glue("Weekly Change in COVID-19 Cases, {params$juris}")),subtitle=glue("January 22, 2020 - {end_Bdate}"))%>%
...
#set hospital title
hosptitle<-data.frame(title=glue("{params$timeframe} Change in COVID-19 Hospital Admissions, {params$juris}"),subtitle=glue("August 01, 2020 - {hospend_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
...
#set death title
deathtitle<-data.frame(title=if_else(params$timeframe=="Monthly",glue("Monthly Change in COVID-19 Deathss, {params$juris}"), glue("Weekly Change in COVID-19 Deaths, {params$juris}")),subtitle=glue("January 22, 2020 - {end_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
...
table_bind <- bind_cols(cases_numbers,
                      deaths_numbers,
                    hosp_juris_tbl|> rename(hosp_daily=hosp_new)|>mutate(hosp_new=rollsum(hosp_daily,7,align="right",fill=NA))|>filter(hosp_date ==max(hosp_date)), 
...
`r params$juris` Weekly COVID Report
