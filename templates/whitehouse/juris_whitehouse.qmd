...
```{r, select-juris,warning=FALSE, message=FALSE, echo=FALSE}
...
#get jurisdictional case data, death data, hospital data, and lab data
case_juris_tbl  <- read_rds(here::here("data/cases_deaths_juris_tbl.rds")) |> 
                            filter(juris == params$juris) |> 
                            select(!contains("deaths"))

death_juris_tbl <- read_rds(here::here("data/cases_deaths_juris_tbl.rds")) |> 
                            filter(juris == params$juris) |> 
                            select(!contains("cases"))

hosp_juris_tbl  <- read_rds(here::here("data/hosp_juris_tbl.rds"))|>
                            filter(juris == params$juris)|>select(-hosp_cum)|>
                            mutate(hosp_cum=sum(hosp_new))
...
#set dates
end_date_raw<-max(case_juris_tbl$cases_date)
...
hospend_date_raw<-max(hosp_juris_tbl$hosp_date,na.rm=TRUE)
...
#set case title
casetitle<-data.frame(title=glue("{params$timeframe} Change in COVID-19 Cases, {state$state.name}"),subtitle=glue("January 22, 2020 - {end_Bdate}"))%>%
...
#define case numbers list format
...
if (timeframe=="Daily"){
#get the latest date
latestcase_date<-max(case_juris_tbl$cases_date,na.rm=TRUE)
#get the 7 day avg on the latest date
latestcase_average<-case_juris_tbl%>%filter(cases_date==max(cases_date,na.rm=TRUE))%>%head(1)%>%pull(cases_avg)
...
lowestavg2021<-case_juris_tbl%>%filter(year(cases_date)==2021)%>%filter(cases_avg==min(cases_avg,na.rm=TRUE))%>%head(1)%>%pull(cases_avg)
#get the date of the lowest 7 day avg of 2021
lowestavg2021date<-case_juris_tbl%>%filter(year(cases_date)==2021)%>%filter(cases_avg==min(cases_avg,na.rm=TRUE))%>%head(1)%>%pull(cases_date)
...
lowestavg2022<-case_juris_tbl%>%filter(year(cases_date)==2022)%>%filter(cases_avg==min(cases_avg,na.rm=TRUE))%>%head(1)%>%pull(cases_avg)
#get the date of the lowest 7 day avg of 2022
lowestavg2022date<-case_juris_tbl%>%filter(year(cases_date)==2022)%>%filter(cases_avg==min(cases_avg,na.rm=TRUE))%>%head(1)%>%pull(cases_date)
...
maxnew<-max(case_juris_tbl$cases_new,na.rm=TRUE)
#plot daily cases curve
caseplot<-ggplot(case_juris_tbl,aes(cases_date)) +
...
#set the color of 7 day avg line
scale_color_manual("",values=c("7-Day Moving Average- New Cases  "="#757575"))+
#set the color of daily case column
...
#set the maximal of the y axis
    scale_y_continuous(limits=c(0,max(case_juris_tbl$cases_new,na.rm=TRUE)*1.3),labels=label_number(big.mark =","))+
    #set theme
...
#get the weekly case data
  case_weekly_epicurve <- case_juris_tbl%>%
    mutate(
...
#get monthly cases data
  case_monthly_epicurve <- case_juris_tbl%>%
...
#set hospital title
hosptitle<-data.frame(title=glue("{params$timeframe} Change in COVID-19 Hospital Admissions, {state$state.name}"),subtitle=glue("August 01, 2020 - {hospend_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
...
hosp_newpeak<-max(hosp_juris_tbl$hosp_new,na.rm=TRUE)
#get the date of the maximal admissions
hosp_newpeakdate<-hosp_juris_tbl%>%filter(hosp_new==hosp_newpeak)%>%head(1)%>%pull(hosp_date)
...
if (timeframe=="Daily"){
#get the latest date
latesthosp_date<-max(hosp_juris_tbl$hosp_date,na.rm=TRUE)
#get the 7 day avg admission on the latest date
latesthosp_average<-hosp_juris_tbl%>%filter(hosp_date==max(hosp_date,na.rm=TRUE))%>%head(1)%>%pull(hosp_avg)
...
hosplowestavg2021<-hosp_juris_tbl%>%filter(year(hosp_date)==2021)%>%filter(hosp_avg==min(hosp_avg,na.rm=TRUE))%>%head(1)%>%pull(hosp_avg)
#get the date of the lowest 7 day avg admission of 2021
hosplowestavg2021date<-hosp_juris_tbl%>%filter(year(hosp_date)==2021)%>%filter(hosp_avg==min(hosp_avg,na.rm=TRUE))%>%head(1)%>%pull(hosp_date)
...
hosplowestavg2022<-hosp_juris_tbl%>%filter(year(hosp_date)==2022)%>%filter(hosp_avg==min(hosp_avg,na.rm=TRUE))%>%head(1)%>%pull(hosp_avg)
#get the date of the lowest 7 day avg admission of 2022
hosplowestavg2022date<-hosp_juris_tbl%>%filter(year(hosp_date)==2022)%>%filter(hosp_avg==min(hosp_avg,na.rm=TRUE))%>%head(1)%>%pull(hosp_date)
...
hospmaxnew<-max(hosp_juris_tbl$hosp_new,na.rm=TRUE)
#plot daily admission curve
hospplot<-ggplot(hosp_juris_tbl,aes(hosp_date)) +
...
#set the maximal value of y axis
    scale_y_continuous(limits=c(0,max(hosp_juris_tbl$hosp_new,na.rm=TRUE)*1.3),labels = label_number(big.mark = ","))+
    #set theme
    theme(axis.title=element_text(size=rel(0.6),face="bold"),
          axis.text=element_text(size = rel(0.6), face = "bold"),
          legend.key.size = unit(0.5, "lines"),
...
#get weekly admissions data
  hosp_weekly_epicurve <- hosp_juris_tbl%>%
...
hosp_monthly_epicurve <- hosp_juris_tbl%>%
...
#set death title
deathtitle<-data.frame(title=glue("{params$timeframe} Change in COVID-19 Deaths, {state$state.name}"),subtitle=glue("January 22, 2020 - {end_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
...
#death numbers
deaths_numbers <- death_juris_tbl %>% 
mutate(
    #weekly deaths
...
latestdeath_date<-max(death_juris_tbl$deaths_date,na.rm=TRUE)
#get the 7 day avg deaht on the latest death date
latestdeath_average<-death_juris_tbl%>%filter(deaths_date==max(deaths_date,na.rm=TRUE))%>%head(1)%>%pull(deaths_avg)
...
deathlowestavg2021<-death_juris_tbl%>%filter(year(deaths_date)==2021)%>%filter(deaths_avg==min(deaths_avg,na.rm=TRUE))%>%head(1)%>%pull(deaths_avg)
#get the date of the lowest 7 day avg deahth of 2021
deathlowestavg2021date<-death_juris_tbl%>%filter(year(deaths_date)==2021)%>%filter(deaths_avg==min(deaths_avg,na.rm=TRUE))%>%head(1)%>%pull(deaths_date)
...
deathlowestavg2022<-death_juris_tbl%>%filter(year(deaths_date)==2022)%>%filter(deaths_avg==min(deaths_avg,na.rm=TRUE))%>%head(1)%>%pull(deaths_avg)
#get the date of the lowest 7 day avg deahth of 2022
deathlowestavg2022date<-death_juris_tbl%>%filter(year(deaths_date)==2022)%>%filter(deaths_avg==min(deaths_avg,na.rm=TRUE))%>%head(1)%>%pull(deaths_date)
...
deathmaxnew<-max(death_juris_tbl$deaths_new,na.rm=TRUE)
#plot the daily deaths
deathplot<-ggplot(death_juris_tbl,aes(deaths_date)) +
...
geom_col(aes(y={deathlowestavg2022}/700,x={deathlowestavg2022date}),color="black")+
    #add the red line of the lowest 7 day avg of 2021
...
scale_fill_manual("",values=c("#B4C7E7"))+
    #set the maximal value of y axis
scale_y_continuous(limits=c(0,max(death_juris_tbl$deaths_new,na.rm=TRUE)*1.3),labels = label_number(big.mark = ","))+ 
    #set axis title
 labs(y     = "New Deaths",
      x     = "Date of Report") +
    #add annotation of the lowest 7 day avg of 2021
    annotate("text",x={deathlowestavg2021date},y={deathlowestavg2021}+{deathmaxnew}/1.5,label=glue("2021:\n LOWEST\n AVERAGE\n \n{deathlowestavg2021_format}"),size=2)+
    #add annotation of the lowest 7 day avg of 2022
    annotate("text",x={deathlowestavg2022date},y={deathlowestavg2022}+{deathmaxnew}/1.0,label=glue("2022:\n LOWEST\n AVERAGE\n \n \n{deathlowestavg2022_format}"),size=1.8)+
    #add annotation of the latest 7 day avg1
...
death_weekly_epicurve <- death_juris_tbl%>%
mutate(
...
death_monthly_epicurve <- death_juris_tbl%>%
