...
```{r load_data}
cases_nat_tbl  <- get_case_death_nat_tbl(username = params$username) |> 
    select(!contains("deaths"))
deaths_nat_tbl <- get_case_death_nat_tbl(username = params$username)|> 
...
### National`r params$timeframe` COVID Report

```{r}
#| echo: false
cases_nat_row <- cases_nat_tbl |> 
    filter(cases_date == max(cases_date))

deaths_nat_row <- deaths_nat_tbl |> 
...
```{r}
create_summary_table(cases_nat_row, 
                     deaths_nat_row,
                     lab_nat_row, 
                     hosp_nat_row) |> 
    flextable()
