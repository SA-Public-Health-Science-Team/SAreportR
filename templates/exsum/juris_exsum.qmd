...
hosp_juris_tbl  <- read_rds(here::here("data/hosp_juris_tbl.rds"))|>
                            ...
                            mutate(hosp_cum=cumsum(hosp_new))

lab_juris_tbl   <- read_rds(here::here("data/lab_juris_tbl.rds"))|> 
                            filter(juris == params$juris)
```

# `r params$juris` COVID-19 Summary
...
#bind tables
table_bind <- bind_cols(
hosp_juris_tbl|> rename(hosp_daily=hosp_new)|>mutate(hosp_new=rollsum(hosp_daily,7,align="right",fill=NA))|>select(-hosp_daily)|>select(-total_patients)|>filter(hosp_date ==max(hosp_date)), 
...
cases_date_raw<-case_juris_tbl%>%filter(cases_date==max(cases_date,na.rm=TRUE))%>%pull(cases_date)
...
casetitle<-data.frame(title=if_else(params$timeframe=="Monthly",glue("Monthly Change in COVID-19 Cases, {params$juris}"), glue("Weekly Change in COVID-19 Cases, {params$juris}")),subtitle=glue("January 22, 2020 - {cases_Bdate}"))%>%
...
casetitle<-title_format_exsum(casetitle)                                   
...
casepeak1<-peak_format(case_juris_tbl,"2020-03-19","2020-06-19")
casepeak2<-peak_format(case_juris_tbl,"2020-06-20","2020-09-21")
casepeak3<-peak_format(case_juris_tbl,"2020-12-21","2021-03-19")
casepeak4<-peak_format(case_juris_tbl,"2021-03-20","2021-06-19")
casepeak5<-peak_format(case_juris_tbl,"2021-06-20","2021-09-30")
casepeak6<-peak_format(case_juris_tbl,"2021-12-22","2022-03-20")
casepeak7<-peak_format(case_juris_tbl,"2022-06-21","2022-09-22")
casepeak8<-peak_format(case_juris_tbl,cases_date_peak_41,cases_date_peak)

casepeakline1<-peak_number(case_juris_tbl,"2020-03-19","2020-06-19")
casepeakline2<-peak_number(case_juris_tbl,"2020-06-20","2020-09-21")
casepeakline3<-peak_number(case_juris_tbl,"2020-12-21","2021-03-19")
casepeakline4<-peak_number(case_juris_tbl,"2021-03-20","2021-06-19")
casepeakline5<-peak_number(case_juris_tbl,"2021-06-20","2021-09-30")
casepeakline6<-peak_number(case_juris_tbl,"2021-12-22","2022-03-20")
casepeakline7<-peak_number(case_juris_tbl,"2022-06-21","2022-09-22")
...
hosp_date_raw<-hosp_juris_tbl%>%filter(hosp_date==max(hosp_date))%>%pull(hosp_date)
hosp_Bdate<-format(hosp_date_raw,format="%B %d, %Y")
hosptitle<-data.frame(title=glue("New Admission of Patients with Confirmed COVID-19, {params$juris}"),subtitle=glue("August 01, 2020 - {hosp_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
select(val)%>%flextable()
hosptitle<-title_format_exsum(hosptitle) 
#get hospital data
hosp_epicurve <- hosp_juris_tbl %>%select(-hosp_cum)%>% filter(hosp_date<=max(hosp_date,na.rm=TRUE)) %>% mutate(
...
deaths_date_raw<-death_juris_tbl%>%filter(deaths_date==max(deaths_date,na.rm=TRUE))%>%pull(deaths_date)
...
deathtitle<-data.frame(title=if_else(params$timeframe=="Monthly",glue("Monthly Change in COVID-19 Deaths, {params$juris}"), glue("Weekly Change in COVID-19 Deaths, {params$juris}")),subtitle=glue("January 22, 2020 - {deaths_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
...
deathtitle<-title_format_exsum(deathtitle)                                   
...
#input peak start date and end date to get data of each peak
deathpeak1<-peak_format(death_juris_tbl,"2020-03-19","2020-06-19")
deathpeak2<-peak_format(death_juris_tbl,"2020-06-20","2020-09-21")
deathpeak3<-peak_format(death_juris_tbl,"2020-12-21","2021-03-19")
deathpeak4<-peak_format(death_juris_tbl,"2021-06-20","2021-09-30")
deathpeak5<-peak_format(death_juris_tbl,"2021-12-22","2022-03-20")
deathpeak6<-peak_format(death_juris_tbl,"2022-06-21","2022-09-22")
deathpeak7<-peak_format(death_juris_tbl,deaths_date_peak_41,deaths_date_peak)

deathpeakline1<-peak_number(death_juris_tbl,"2020-03-19","2020-06-19")
deathpeakline2<-peak_number(death_juris_tbl,"2020-06-20","2020-09-21")
deathpeakline3<-peak_number(death_juris_tbl,"2020-12-21","2021-03-19")
deathpeakline4<-peak_number(death_juris_tbl,"2021-06-20","2021-09-30")
deathpeakline5<-peak_number(death_juris_tbl,"2021-12-22","2022-03-20")
deathpeakline6<-peak_number(death_juris_tbl,"2022-06-21","2022-09-22")
...
lab_date<-format(lab_date_raw,format="%b %d, %Y")
labtitle<-data.frame(title=glue("{params$timeframe} SARS-CoV-2 NAAT Percent Test Positivity and Test Volume, {params$juris}"),subtitle=glue("March 01, 2020 - {lab_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
select(val)%>%flextable()
labtitle<-title_format_exsum(labtitle) 
