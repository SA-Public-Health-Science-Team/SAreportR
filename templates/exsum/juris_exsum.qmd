...
#get jurisdictional case, death, hospital, and lab data
...
hosp_juris_tbl  <- read_rds(here::here("data/hosp_juris_tbl.rds"))|>
                            filter(juris == params$juris)|>
                      select(-hosp_cum)|>mutate(hosp_cum=sum(hosp_new))
...
# COVID-19 Summary, `r params$juris`

```{r juris-summary}
#| tbl-cap: "juris-summary"

#get jurisdictional summary table
table_bind <- bind_cols(
            hosp_juris_tbl|> filter(hosp_date ==max(hosp_date)),
           case_juris_tbl |>  filter(cases_date ==max(cases_date)),
                      death_juris_tbl |> filter(deaths_date == max(deaths_date)),
...
table_bind |>  
    ...
    cases_cum=cases_cum|>format(justify = "right",big.mark   =","),
         ...
         deaths_avg=deaths_avg|>format(justify = "right",big.mark   = ","))|>
   select(ends_with(c("date","cum","new",'avg',"change","7cum"))) |>
  create_summary_table() |>subset(select=c(1,3,4,5,6,2))|>
  flextable()|>
summary_table_format_function()|>
align_bold_function()
```
...
#get the case dates
cases_date_raw<-max(case_juris_tbl$cases_date)
...
dataset<-add_header_row(x=dataset,top=TRUE,values=glue("{params$timeframe} Change in COVID-19 Cases, {params$juris}"),colwidths=c(7))
...
#get the maximal daily cases of the date range
...
#get the mximal 7 day avg case
peak_7_average=max(peak$cases_avg,na.rm=TRUE)
#get the date of the mximal 7 day avg case
peak7avgdate<-peak%>%filter(cases_avg==peak_7_average)%>%
    mutate(cases_date=min(cases_date,na.rm=TRUE))%>%head(1)%>%pull(cases_date)
#compare the maximal 7 day avg to the current 7 day avg
...
#combine each peak to get case peak flextable
...
peak14avgdate<-peak%>%filter(cases_14avg==peak_14_average)%>%
    mutate(cases_date=min(cases_date,na.rm=TRUE))%>%head(1)%>%pull(cases_date)
#get the percent change of the maximal 14 day avg and the current 14 day avg
...
#set the color of 7 day avg case line
...
cases_monthly_epicurve <- case_juris_tbl%>%
...
#get hospital dates
hosp_date_raw <- hosp_juris_tbl  %>% arrange(desc(hosp_date)) %>% head(1)  %>% pull(hosp_date) 
...
#get hospital data
hosp_epicurve <-hosp_juris_tbl%>%filter(hosp_date<=max(hosp_date,na.rm=TRUE)) %>% 
mutate(
#total admissions
hosp_cumsum=sum(hosp_new),
#weekly admissioins
...
#percent change of 14 day admission
...
#format numbers
hosp_cum = hosp_cum %>% format(justify = "right",big.mark = ","),
hosp_cumsum = hosp_cumsum %>% format(justify = "right",big.mark = ","),
...
totalnumber=hosp_numbers$hosp_cumsum,
...
hosppeak<-(data.frame(c("1st Peak","2nd Peak","Latest Peak")))%>%cbind(hosppeak_raw)%>%flextable()
...
#create death table
deaths_epicurve <- death_juris_tbl  %>% 
mutate(
#weekly deaths
...
dataset<-add_header_row(x=dataset,top=TRUE,values=c("s"),colwidths=c(7))
dataset<-add_header_row(x=dataset,top=TRUE,values=glue("January 22, 2020 - {cases_Bdate}"),colwidths=c(7))
dataset<-add_header_row(x=dataset,top=TRUE,values=glue("{params$timeframe} Change in COVID-19 Deaths, {params$juris}"),colwidths=c(7))
dataset<-fontsize(x=dataset,i=1,size=16,part="header")
dataset<-fontsize(x=dataset,i=2,size=8,part="header")
dataset<-color(x=dataset,i=3,color="white",part="header")
dataset<-hrule(dataset,rule="exact",part="body")
...
#get the data of the maximal 7 day avg death
peak7avgdate<-peak%>%filter(deaths_avg==peak_7_average)%>%
    mutate(deaths_date=min(deaths_date,na.rm=TRUE))%>%head(1)%>%pull(deaths_date)
#percent change of the maximal 7 day avg and the current 7 day avg death
...
#get current weekly deaths data
...
peakdate<-peak%>%filter(deaths_cum7==peakdeaths)%>%
    mutate(peakactualdate=min(deaths_date,na.rm=TRUE))%>%head(1)
...
peak14avgdate<-peak%>%filter(deaths_14avg==peak_14_average)%>%
    mutate(peakactualdate2=min(deaths_date,na.rm=TRUE))%>%head(1)
#percent change of the maximal 14 day avg death and the current 14 day avg death
peakchange=(death_current_14_average-peak_14_average)/peak_14_average
#format numbers
Deaths=label_number(big.mark=",",accuracy=1)(peakdeaths)
Dates=format(peakdate$peakactualdate,format="%d-%b-%y")
Death=label_number(big.mark=",",accuracy=1)(peak_14_average)
Date=format(peak14avgdate$peakactualdate2,format="%d-%b-%y")
Change=label_percent(decimal.mark = ".",accuracy = .1)(peakchange)
...
#set color of 7 day avg death line
...
#add the line of 14 day avg death 
geom_line(aes(y=deaths_14avg*7,group=1,color="14-Day Average Death    "),size=1.3)+
#set the color of the line of 14 day avg death 
...
return(deathplot)
     ...
     deaths_monthly_epicurve <- death_juris_tbl%>%
...
mutate(testpos_dat=min(testpos_date,na.rm=TRUE))%>%head(1)%>%pull(testpos_date)
...
mutate(testvol_avg_date=min(testvol_avg_date,na.rm=TRUE))%>%head(1)%>%
pull(testvol_avg_date)
...
#percent change of the maximal 14 day avg test volume and current 14 day avg test volume
