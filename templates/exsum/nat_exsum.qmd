...
lab_date<-format(lab_date_raw,format="%b %d, %Y")
labtitle<-data.frame(title=glue("{params$timeframe} SARS-CoV-2 NAAT Percent Test Positivity and Test Volume, United States"),subtitle=glue("March 01, 2020 - {lab_Bdate}"))%>%
pivot_longer( cols = everything(),
                 names_to = "names",
                values_to = "val", 
               values_transform = list(val = as.character))%>%
