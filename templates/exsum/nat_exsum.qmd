...
params:
  ...
  zero_report_states: !r c(NULL)
  reporting_cadence: !r c(NULL)
  historical_rec: !r c(NULL)
  backlog: !r c(NULL)
code-fold: true
---
...
historical_bullets <- cases_raw %>%
  ...
  pivot_longer(cols    =  c(historical_new_total_cases,
                ...
                names_to = "indicator") %>%
  mutate(indicator = indicator %>% str_remove("historical_new_total_")) %>%
...
table_bind |>  
  ...
  flextable()|>summary_table_format_function()|>align_bold_function()
...
#get the case dates
cases_date_raw<-max(cases_nat_tbl$cases_date)
...
dataset<-add_header_row(x=dataset,top=TRUE,values=glue("{params$timeframe} Change in COVID-19 Cases, US"),colwidths=c(7))
...
cases_weekly_epicurve <- cases_epicurve%>%
    filter(wday(cases_date)==wday(cases_date_raw))

#define case peaks function
case_peaks_list<-function(timeframe){
if (timeframe=="Daily"){
#get current 7 day avg case
...
#get the data between the date range
peak<-cases_epicurve%>%
...
#create dataframe
return(data.frame("Cases"=Cases,"Dates"=Week,"1"="1","Case"=Case,"Date"=Date,"%Change vs Current Avg"=Change))
                                               } 
...
casepeak_raw<-casepeak1%>%rbind(casepeak2)%>%rbind(casepeak3)%>%rbind(casepeak4)%>%
rbind(casepeak5)%>%rbind(casepeak6)
...
peakdate<-peak%>%filter(cases_cum7==peakcases)%>%
    mutate(cases_date=min(cases_date,na.rm=TRUE))%>%head(1)%>%pull(cases_date)
#get the maximal 14 day avg case
peak_14_average=max(peak$cases_14avg,na.rm=TRUE)
#get the date of the maximal 14 day avg case
...
#set color of 7 day avg line
scale_color_manual(" ",values=c("7-Day Average Case    "="#E1C233"))+
#set color of column
...
scale_color_manual(" ",values=c("14-Day Average Case       "="#E1C233"))+
#set color of column
scale_fill_manual(" ",values="#573B92")+
#set the maximal value of y axis and set the secondary y axis
...
cases_monthly_epicurve <- cases_nat_tbl%>%
...
#get hospital dates
hosp_date_raw <-hosp_nat_tbl%>%arrange(desc(hosp_date))%>%head(1)%>%pull(hosp_date) 
...
#get hospital data
hosp_epicurve <- hosp_nat_tbl  %>% filter(hosp_date<=max(hosp_date,na.rm=TRUE)) %>%
mutate(
#weekly admissions
...
#define hospital peak function
hosp_peak_function<-function(startdate,enddate){
#filter peak date range
peak<-hosp_epicurve%>%
...
#add the column of daily admissions
...
#add the column of monthly admissions
...
#create death table
deaths_epicurve <- deaths_nat_tbl  %>% 
mutate(
   #weekly deaths
...
deaths_weekly_epicurve <- deaths_epicurve%>%
    filter(wday(deaths_date)==wday(cases_date_raw))
#define death peak format function
deathpeaks_format_function<-function(dataset){
dataset<-add_header_row(x=dataset,top=TRUE,values=c("s"),colwidths=c(7))
dataset<-add_header_row(x=dataset,top=TRUE,values=glue("January 22, 2020 - {cases_Bdate}"),colwidths=c(7))
dataset<-add_header_row(x=dataset,top=TRUE,values=glue("{params$timeframe} Change in COVID-19 Deaths, US"),colwidths=c(7))
dataset<-fontsize(x=dataset,i=1,size=16,part="header")
dataset<-fontsize(x=dataset,i=2,size=8,part="header")
dataset<-color(x=dataset,i=3,color="white",part="header")
dataset<-hrule(dataset,rule="exact",part="body")
...
#get death current 14 day average death
...
#get the date of the maximal weekly deaths
peakdate<-peak%>%filter(deaths_cum7==peakdeaths)%>%
    mutate(deaths_date=min(deaths_date,na.rm=TRUE))%>%head(1)%>%pull(deaths_date)
#get the maximal 14 day avg death
peak_14_average=max(peak$deaths_14avg,na.rm=TRUE)
#get the date of the maximal 14 day avg death
peak14avgdate<-peak%>%filter(deaths_14avg==peak_14_average)%>%
    mutate(deaths_date=min(deaths_date,na.rm=TRUE))%>%head(1)%>%pull(deaths_date)
#percent change of the maximal 14 day avg death and current 14 day avg death
...
#add the 14 day avg death line
geom_line(aes(y=deaths_14avg*7,group=1,color="14-Day Average Death    "),size=1.3)+
#set the color of the 14 day avg death line
scale_color_manual(" ",values=c("14-Day Average Death    "="#E66C37"))+
#set the color of column
scale_fill_manual(" ",values="#08519C")+
#set the maximal value of y axis and set the secondary y axis
...
deaths_monthly_epicurve <- deaths_nat_tbl%>%
...
#get lab data
lab_epicurve <- lab_nat_tbl%>%filter(testpos_date<=max(testpos_date,na.rm=TRUE))%>% filter(testpos_date>=as.Date("2020-03-14",format="%Y-%m-%d"))%>%
mutate(
  #remove "," in the character of testvol_cum
  testvol_cum=gsub(",","",testvol_cum),
  #remove "," in the character of testvol_avg
  testvol_avg=gsub(",","",testvol_avg),
  #remove "%" in the character of testpos_avg
  testpos_avg=gsub("%","",testpos_avg))%>%
mutate(
       #convert testvol_cum to double
       testvol_cum=as.double(testvol_cum),
       #convert testvol_avg to double
       testvol_avg=as.double(testvol_avg),
       #convert testpos_avg to numeric
       testpos_avg=as.numeric(testpos_avg)/100,
       #calculate positive tests number
...
#percent change of 14 day avg test volume
...
testpos_14avg=rollsum(postest,14,align="right",fill=NA)/
              rollsum(new_test_vol,14,align="right",fill=NA),
#prior 7 day avg percent positivity
testpos_prior_7avg= (rollsum(postest,7,align = "right",fill = NA)%>%
                        lag(order_by=testpos_date,n=7))/
                    (rollsum(new_test_vol,7,align="right",fill=NA)%>%
                        lag(orer_by=testpos_date,7)),
#prior 14 day avg percent positivity
testpos_prior_14avg=(rollsum(postest,14,align = "right",fill = NA)%>%
                        lag(order_by=testpos_date,n=14))/
                    (rollsum(new_test_vol,14,align ="right",fill=NA)%>%
                        lag(orer_by=testpos_date,14)),
#percent change of 14 day avg percent positivity
...
#get lab weekly data
lab_weekly_epicurve <- lab_epicurve%>%
    filter(wday(testpos_date)==wday(lab_date_raw))

#define lab peaks function
lab_peaks_list<-function(timeframe){
if (timeframe=="Daily"){
#get current lab data
lab_current_7_average<-lab_epicurve%>%
     filter(testpos_date==max(testpos_date,na.rm=TRUE))   
#define lab peak function
...
peaktestvolavgdate<-peak%>%filter(testvol_avg==peaktestvolavg)%>%
   mutate(testvol_avg_date=min(testvol_avg_date,na.rm=TRUE))%>%
   head(1)%>%pull(testvol_avg_date)
...
#create dataframe
return(data.frame("Peak"="Peak","Positivity"=pos,"Date"=posdate,"1"="1","Positivity"=posavg,"Date"=posavgdate,"Change vs. Current 7-Day Average"=poschange,"2"="2","Peak"="Peak","Test Vol"=testvol,"Date"=testvoldate,"3"="3","Test Vol"=testvolavg,"Date"=testvolavgdate,"Change vs. Current Average"=testvolchange))
}
#get each lab peak and combine to form flextable
...
#get current weekly lab data
lab_current_14_average<-lab_weekly_epicurve%>%
    filter(testpos_date==max(testpos_date,na.rm=TRUE))
...
peakposdate<-peak%>%filter(testpos_avg==peakpos)%>%
   mutate(testpos_date=min(testpos_date,na.rm=TRUE))%>%head(1)%>%pull(testpos_date)
#get the maximal 14 day avg percent positivity
peakposavg=max(peak$testpos_14avg,na.rm=TRUE)
#get the date of the maximal 14 day avg percent positivity
peakposavgdate<-peak%>%filter(testpos_14avg==peakposavg)%>%
   mutate(testpos_date=min(testpos_date,na.rm=TRUE))%>%head(1)%>%pull(testpos_date)
#percent change of the maximal 14 day avg percent positivity and current 14 day avg percent positivity
peakposchange=(lab_current_14_average$testpos_14avg-peakposavg)/peakposavg
#get the maximal weekly test volume
peaktestvol=max(peak$test_cum7,na.rm=TRUE)
#get the date of the maximal weekly test volume
peaktestvoldate<-peak%>%filter(test_cum7==peaktestvol)%>%
   mutate(testpos_date=min(testpos_date,na.rm=TRUE))%>%head(1)%>%pull(testpos_date)
#get the maximal 14 day avg test volume
peaktestvolavg=max(peak$testvol_14avg,na.rm=TRUE)
#get the date of the maximal 14 day avg test volume
peaktestvolavgdate<-peak%>%filter(testvol_14avg==peaktestvolavg)%>%
    mutate(testvol_avg_date=testvol_avg_date)%>%head(1)%>%pull(testvol_avg_date)
#percent change of 14 day avg test volume
peaktestvolchange=(lab_current_14_average$testvol_14avg-peaktestvolavg)/
                                                        peaktestvolavg
#format number
